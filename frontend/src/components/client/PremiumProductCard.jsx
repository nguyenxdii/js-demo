  ...
  return (
    <div className="group h-full">
      <Card
        ...
        cover={
          <div className="relative aspect-[4/5] overflow-hidden bg-[#fafafa] flex items-center justify-center group/img">
            ...
            <div className="absolute top-4 left-4 flex flex-col gap-2">
              {discount > 0 && (
                <div className="bg-orange-500 text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-lg shadow-orange-500/30">
                  -{discount}%
                </div>
              )}
              {product.isHot && (
                <div className="bg-black text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-lg shadow-black/30 uppercase tracking-tighter">
                  HOT
                </div>
              )}
            </div>
        ...
        bodyStyle={{ padding: "20px", paddingTop: "12px" }}
      >
        <div className="flex flex-col gap-2.5">
          ...
          <Link to={`/product/${product.slug}`}>
            <Title
              level={5}
              className="!m-0 !text-sm !font-medium text-slate-800 line-clamp-2 hover:text-orange-500 transition-colors h-10 leading-relaxed"
            >
              {product.name}
            </Title>
          </Link>
