...
const SectionManagement = () => {
  ...
  const handleAddProductToSection = async (productId) => {
    try {
      const updatedSectionRaw = await sectionService.addProductToSection(
        currentSection.id,
        productId,
      );
  ...
  const handleRemoveProductFromSection = async (productId) => {
    try {
      const updatedSectionRaw = await sectionService.removeProductFromSection(
        currentSection.id,
        productId,
      );
      const updatedSection = normalizeSection(updatedSectionRaw);
  ...
  const columns = [
    { title: "", key: "sort", width: 50, align: 'center' },
    {
      title: "Tên khung",
      dataIndex: "name",
      key: "name",
    ...
    },
    {
      ...
      render: (type) => {
        ...
        if (type === "FLASH_SALE_1") {
          color = "orange";
          label = "Flash Sale v1 ⚡";
        }
        if (type === "FLASH_SALE_2") {
          color = "volcano";
          label = "Flash Sale v2 🔥";
        }
        if (type === "FLASH_SALE_3") {
          color = "magenta";
          label = "Flash Sale v3 💎";
        }
        if (type === "NEW_ARRIVAL") {
        ...
        return (
          <Tag color={color} className="rounded-md border-none px-3 text-[10px] font-bold uppercase">
            {label}
          </Tag>
        );
      },
    },
    {
      ...
      render: (config) =>
        config?.active ? (
          <Tag color="volcano" className="font-bold border-none text-[10px]">
            -{config.discountPercentage}%
          </Tag>
        ) : (
          <Tag color="default" className="border-none text-[10px]">KHÔNG</Tag>
        ),
    },
    {
      ...
      render: (_, record) => (
        <Space direction="vertical" size={0}>
          <Tag color="orange" className="font-bold border-none">
            {record.layoutType}
          </Tag>
          {record.layoutType === "FLASH_SALE" && (
            <>
              <Text type="secondary" style={{ fontSize: "10px" }}>
                Từ: {dayjs(record.startDate).format("DD/MM HH:mm")}
              </Text>
              <Text type="danger" style={{ fontSize: "10px" }}>
                Đến: {dayjs(record.endDate).format("DD/MM HH:mm")}
              </Text>
            </>
          )}
        </Space>
      ...
      render: (isActive, record) => (
        <Switch
          ...
          onChange={async (checked) => {
            try {
              await sectionService.updateSection(record.id, {
                active: checked,
              });
    ...
    <div className="p-6 overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-8 border-b border-gray-100 pb-4">
          <div>
            ...
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mt-1 block">
              Sắp xếp và quản lý các khung sản phẩm trên trang chủ
            </span>
          </div>
          <Button
            type="primary"
            ...
            icon={<PlusOutlined />}
            onClick={handleAdd}
            className="shadow-md shadow-orange-500/10 rounded-xl px-6 h-10 text-[12px] font-bold uppercase tracking-wider bg-orange-500 hover:bg-orange-600 border-none"
          >
            Thêm Khung Mới
          </Button>
        </div>
      ...
      <Modal
        title={
          <div className="pt-2 pb-4 border-b border-gray-100 mb-4 flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${editingSection ? 'bg-indigo-50 text-indigo-500' : 'bg-orange-50 text-orange-500'}`}>
               {editingSection ? <EditOutlined /> : <PlusOutlined />}
            </div>
            <div>
              <Text type="secondary" className="text-[9px] uppercase font-bold tracking-[0.2em] block leading-none mb-1">Cấu hình hiển thị</Text>
              <h1 className="text-lg font-black uppercase tracking-tight text-slate-800 m-0">
                {editingSection ? "Cập nhật khung" : "Tạo khung sản phẩm"}
              </h1>
            </div>
          </div>
        }
      ...
      >
        <Form form={form} layout="vertical" className="mt-4">
          ...
          >
            <Input
              className="rounded-lg h-10 font-bold border-gray-300"
              placeholder="Ví dụ: BỘ SƯU TẬP GIÀY NIKE MỚI"
            />
          </Form.Item>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              <Select className="h-10">
                ...
                <Select.Option value="FLASH_SALE">
                  ⚡ Flash Sale (Countdown)
                </Select.Option>
                <Select.Option value="NEW_ARRIVAL">
                  🆕 Hàng Mới Về
                </Select.Option>
                <Select.Option value="BEST_SELLER">
                  🏆 Bán Chạy Nhất
                </Select.Option>
              </Select>
            </Form.Item>
          ...
          <Form.Item
            ...
            shouldUpdate={(prev, current) =>
              prev.layoutType !== current.layoutType
            }
          >
            {({ getFieldValue }) =>
              getFieldValue("layoutType") === "FLASH_SALE" && (
                <Form.Item
                  name="rangeTime"
                  label={
                    <Text className="text-[10px] font-bold uppercase tracking-widest text-gray-400">
                      Thời gian diễn ra Sale
                    </Text>
                  }
                  rules={[{ required: true }]}
                >
          ...
          <div className="p-4 bg-orange-50/30 rounded-xl border border-orange-100 mb-6 mt-4">
            <div className="flex items-center justify-between mb-4">
              <Text className="text-[10px] font-bold uppercase tracking-widest text-orange-600">
                Cấu hình giảm giá chung
              </Text>
            ...
            <Form.Item
              noStyle
              shouldUpdate={(prev, current) =>
                prev.hasDiscount !== current.hasDiscount
              }
            >
              {({ getFieldValue }) =>
                getFieldValue("hasDiscount") && (
      ...
      <Modal
        title={
          <div className="pt-2 pb-4 border-b border-gray-100 mb-4 flex items-center gap-3">
            ...
            <div>
              <Text type="secondary" className="text-[9px] uppercase font-bold tracking-[0.2em] block leading-none mb-1">Danh sách hàng hóa</Text>
              <h1 className="text-lg font-black uppercase tracking-tight text-slate-800 m-0 line-clamp-1">
      ...
      >
        <div className="mb-4">
          <Input
            ...
            placeholder="Tìm sản phẩm để thêm vào khung..."
          ...
          <List
            ...
            dataSource={(() => {
              return allProducts.filter((p) => {
                ...
                // Hiển thị sản phẩm khớp với tìm kiếm
                return matchSearch;
              });
            })()}
            renderItem={(item) => {
              const isInSection = currentSection?.products?.some(
                (p) => (p._id || p.id || p) === item.id,
              );
              return (
                <List.Item
                  actions={[
                    isInSection ? (
                      <Button
                        danger
                        type="link"
                        onClick={() => handleRemoveProductFromSection(item.id)}
                      ...
                      <Button
                        type="link"
                        onClick={() => handleAddProductToSection(item.id)}
                        className="font-semibold text-xs uppercase text-primary"
                      >
                        Thêm vào
                      </Button>
                    ),
                ...
                >
                  <List.Item.Meta
                    ...
                    description={
                      <Text className="text-[11px] font-semibold text-primary uppercase">
