...
const importData = async () => {
  try {
    ...
    console.log("🖼️ Đang tạo Banner trang chủ...");
    if ((await Banner.countDocuments()) === 0) {
      const sampleBanners = [
        ...
        {
          title: "Bộ Sưu Tập Giày Chạy Bộ Mới",
          imageUrl:
            "https://images.unsplash.com/photo-1595950653106-6c9ebd614d3a?q=80&w=1887&auto=format&fit=crop",
          linkUrl: "/category/giay-chay-bo-running",
          position: "HOME_MAIN",
          active: true,
        },
        {
          title: "Vợt Cầu Lông Đỉnh Cao 2024",
          imageUrl:
            "https://static.fbshop.vn/wp-content/uploads/2024/03/vot-cau-long-yonex-astrox-88d-pro-bom-tan-dau-nam-2024-1-1024x1024.jpg",
          linkUrl: "/category/vot-cau-long",
          position: "HOME_MAIN",
          active: true,
        },
      ];
      await Banner.insertMany(sampleBanners);
      console.log("✅ Đã tạo 3 Banner mẫu.");
    }
    ...
    const brandsData = [
      ...
      },
      {
        ...
        slug: "yonex",
        logoUrl:
          "https://i.pinimg.com/736x/19/64/d0/1964d07c9e625f0804ca7d890beacfa7.jpg",
      },
      {
        name: "Puma",
        slug: "puma",
        logoUrl:
          "https://thumbs.dreamstime.com/b/vinnitsa-ukraine-october-puma-sport-brand-logo-icon-vinnitsa-ukraine-october-puma-sport-brand-logo-icon-vector-editorial-260965684.jpg",
      },
      {
        name: "Asics",
        slug: "asics",
        logoUrl:
          "https://i.pinimg.com/736x/67/05/70/6705701c0a77e399932e4b171c11e716.jpg",
      },
      {
        name: "Victor",
        slug: "victor",
        logoUrl:
          "https://images.seeklogo.com/logo-png/16/1/victor-sport-logo-png_seeklogo-168727.png",
      },
      {
        name: "Lining",
        slug: "lining",
        logoUrl:
          "https://inkythuatso.com/uploads/thumbnails/800/2021/12/logo-lining-inkythuatso-21-14-57-44.jpg",
      },
      {
        name: "Under Armour",
        slug: "under-armour",
    ...
    for (const b of brandsData) {
      await Brand.findOneAndUpdate({ name: b.name }, b, {
        upsert: true,
        new: true,
      });
    }
    ...
    if (existingProductsCount < 10) {
      ...
      console.log("✅ Đã tạo 150 sản phẩm mẫu.");
    } else {
      products = await Product.find().limit(100);
      console.log(
        `ℹ️ Đã có ${existingProductsCount} sản phẩm, bỏ qua bước tạo mới.`,
      );
    }

    // 4. Tạo Section trang chủ đa dạng
    console.log("📺 Đang bổ sung Section trang chủ...");
    if ((await Section.countDocuments()) < 5) {
      const sectionsData = [
        {
          title: "Siêu Ưu Đãi Flash Sale",
          type: "FLASH_SALE_1",
        ...
        },
        {
          ...
          order: 2,
          products: products.slice(10, 20).map((p) => p._id),
        },
        {
          title: "Vợt Cầu Lông Đỉnh Cao",
          type: "CUSTOM",
          layoutType: "STANDARD",
          order: 3,
          products: products.slice(20, 30).map((p) => p._id),
        },
        {
          ...
          order: 4,
          products: products.slice(30, 40).map((p) => p._id),
        },
      ];
      await Section.insertMany(sectionsData);
      console.log("✅ Đã tạo thêm 4 Section mẫu.");
    }
    ...
    console.log(
      "🎉 XONG! Tất cả dữ liệu (Admin, Banner, Section, Đơn hàng) đã sẵn sàng.",
    );
    process.exit(0);
  } catch (error) {
